...
export function saveBestLevel(level) {
  try {
    ...
    if (curr == null || level > curr) {
      localStorage.setItem(STORAGE_KEY_LEVEL, String(level));
      return true;
    }
  } catch (e) {}
  return false;
...
export function updateBestBadge(bump = false) {
  ...
  const bestStr = t == null ? '—' : String(t);
  const bestTimeEl = document.getElementById('bestTime');
  if (bestTimeEl) bestTimeEl.textContent = bestStr;
  const lobbyBestEl = document.getElementById('lobbyBest');
  if (lobbyBestEl) lobbyBestEl.textContent = t == null ? 'Nivel 1' : `Nivel ${t}`;
