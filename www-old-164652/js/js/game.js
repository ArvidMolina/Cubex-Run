...
export const SLIME_MIN_BFS_DIST     = 7;
export const SLIME_FALLBACK_DELAY_MS = 1500;
...
function tickInner(now) {
  ...
  if (state.phase === 'playing' && !stopped) {
    ...
    if (state.slimeAlive && state.slime) {
      const tFade = (now - state.slimeAppearTime) / (cfg.FADE_IN_S * 1000);
      ...
      if (op >= 1) {
        ...
        if (state.enemyTarget) {
          ...
          if (tlen > 0.05) moveEntity(state.slime, tdx / tlen, tdz / tlen, dt, cfg.ENEMY_SPEED);
        }
      }
    }
    ...
    if (state.phase === 'playing' &&
      ...
      document.getElementById('winBest').textContent =
        (isNewBestLevel ? '⭐ ' : '') + 'Nivel ' + String(loadBestLevel() ?? state.level);
